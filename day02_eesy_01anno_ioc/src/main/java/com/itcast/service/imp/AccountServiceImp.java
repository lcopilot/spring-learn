 ...
 *
 * 曾经XML的配置:
 * <bean id="accountService" class="com.itcast.service.imp.AccountServiceImp"
 ...
 *		是spring框架为我们提供明确的三层使用的注解,使我们的三层对象更加清晰
 *
 * 用于注入数据的
 * 		他们的作用就和在xml配置文件中的bean标签中写一个<property>标签的作用是一样的
 * 		@Autowired:
 * 			作用:自动按照类型注入,只要容器中有唯一的一个bean对象类型和要注入的变量类型匹配,就可以注入成功
 * 				如果ioc容器中没有任何bean的类型和要注入的变量类型,则报错
 * 				如果ioc容器中有多个类型匹配时:
 * 			出现位置:
 * 				可以是变量上,也可以是方法上
 * 			细节:
 ...
 * 和生命周期相关的(了解)
 *		他们作用就和在bean标签中使用init-method和destroy-methode的作用是一样的
 * 		@PreDestroy:
 * 			作用: 用于指定销毁方法
 * 		@PostConstruct:
 * 			作用: 用于指定初始化方法
